...
import { AnimationDefaultDuration, ProductRating, ProductTabsVariants, animationDefaultVariants } from "@/common/constants";
...
export const ProductFeedbackTab: FC<Props> = ({ selectedTab, ...props }) => {
  ...
  return (
    ...
    >
      {feedbackData.length ? (
        feedbackData.map(credentials => (
          <ProductFeedback rating={ProductRating.MIN} key={credentials.id} isRating={isRating} {...credentials} />
        ))
      ) : (
        <NoMatches />
      )}
